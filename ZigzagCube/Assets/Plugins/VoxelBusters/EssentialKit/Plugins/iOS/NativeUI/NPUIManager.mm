...
- (void) showDatePicker:(NPUIDatePickerMode) mode withInitialDate:(long) initialDateTime withMinimumDate:(long) min withMaximumDate:(long) max  tagPtr:(void*)tagPtr {
    ...
    UIAlertController *controller = [UIAlertController create:   (UIDatePickerMode)mode
    ...
    NSLog(@"Date picker is not currently supported on this target.");
...
- (void)alertController:(UIAlertController*)alertController didSelectAction:(UIAlertAction*)action withInputFields:(NSArray<UITextField*>*) textfields
{
    ...
    //Get the input text fields text values as string array
    NSMutableArray *inputValues = nil;
    if([alertController.textFields count] > 0)
    {
        inputValues = [NSMutableArray array];
        for (UITextField* each in alertController.textFields)
        {
            [inputValues addObject:each.text];
        }
    }
    ...
    {
        _alertActionSelectCallback((__bridge void*)retainAlertController, (int)selectedActionIndex,NPCreateArrayOfCString(inputValues));
    }
}
...
- (void)presentationControllerDidDismiss:(UIPresentationController*)presentationController
{
    ...
    BOOL isDatePicker = [controller.view.subviews indexOfObjectPassingTest:^BOOL(id obj, NSUInteger idx, BOOL *stop) {
        return [obj isKindOfClass:[UIDatePicker class]];
    }] != NSNotFound;
    
    if (controller != nil && [controller class] == [UIAlertController class] && !isDatePicker)
    {
        ...
        if (targetAction)
        {
            [self alertController:alertController didSelectAction:targetAction withInputFields:alertController.textFields];
        }
    }
