...
namespace VoxelBusters.EssentialKit.NativeUICore.iOS
{
    internal sealed class NativeDatePicker : NativeDatePickerInterfaceBase, INativeDatePickerInterface
    {
        ...
        public override void SetKind(DateTimeKind value)//TODO: Fix kind on iOS
        {
            m_kind = value;
        }
        ...
        {
            long initialEpoch = GetEpochInSeconds(m_initialDate?.ToUniversalTime(), DateTimeOffset.Now.ToUnixTimeSeconds());
            long minimumEpoch = GetEpochInSeconds(m_minimumDate?.ToUniversalTime());
            long maximumEpoch = GetEpochInSeconds(m_maximumDate?.ToUniversalTime());
