...
using Utility = VoxelBusters.CoreLibrary.NativePlugins.Android.Utility;
...
namespace VoxelBusters.EssentialKit.SharingServicesCore.Android
{
    public sealed class SocialShareComposer : NativeSocialShareComposerBase, INativeSocialShareComposer
    {
        ...
        {
            Utility.TakeScreenshot((byte[] data, string mimeType) =>
            {
                m_instance.AddAttachment(new NativeBytesWrapper(data), MimeType.kJPGImage, DateTime.Now.Ticks.ToString() + ".jpg");
            });
        }

        //[Obsolete("Use AddImage(byte[] imageData, string mimeType, string fileNameWithoutExtension) instead")] //TODO
        ...
        /*public override void AddImage(byte[] imageData, string mimeType, string fileNameWithoutExtension = null)
        {
            m_instance.AddAttachment(new NativeBytesWrapper(imageData), mimeType, (string.IsNullOrEmpty(fileNameWithoutExtension) ? DateTime.Now.Ticks.ToString() : fileNameWithoutExtension) + "." + MimeType.GetExtensionForType(mimeType));
        }*/
